...
1. **界面顶部挂一条「演示数据」横幅。** 知识库来自虚构公司「云启科技」，
   员工档案、社保、工单全是造的。不标出来，看的人有理由以为这是
   真接了企业系统——**一个讲隐私和诚实的产品，不该在这件事上含糊。**

2. **把演示员工的入职日期滚回 30 天窗口内。**
   种子里的入职日期是写死的，过一段时间新人就"毕业"了：
   实测再过 20 天林小雨就到第 40 天，时间线全变已完成、主动推送不再触发，
   整个"有状态的 30 天"叙事当场塌掉。演示站要能在任何一天打开都对。

   ⚠️ 这是**演示专用**的数据改写。真实部署必须关掉——
   一个会自己改员工入职日期的 HR 系统是灾难。所以默认是 0。
"""
